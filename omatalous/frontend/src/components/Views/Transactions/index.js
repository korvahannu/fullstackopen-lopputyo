            ...
            <Box sx={{ mb: '16px', display:'flex' }}>
...

                {
                    selected.length < 1
                        ? null
                        : selected.length === 1
                            ? <Button variant='outlined' sx={{mr:4}} onClick={() => setShowEditTransactionDialog(true)} startIcon={<EditIcon />}> Edit selected</Button>
                            : <Button variant='outlined' sx={{mr:4}} color='error' onClick={deleteSelected} startIcon={<DeleteForeverIcon />}> Delete selected</Button>
                }

                <Box sx={{flexGrow:1}}/>

                <Select size='small' sx={{ minWidth: '150px', mr: 4 }} variant='standard' defaultValue={'all'}
